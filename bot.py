...
def contact_info_text():
    text = "📩 /contact — Admin se baat karo"
    if ADMIN_LINK:
        text += f"\n📲 Ya seedha: {ADMIN_LINK}"
    return text
...
async def process_upload(client, message: Message, status_msg, file_unique_id,
    ...
    try:
        ...
        if yt_link:
            # ✅ UPDATED: file_unique_id bhi save ho raha hai ab
            await db.save_video(
                title=title,
                caption=caption,
                yt_link=yt_link,
                ...
                file_unique_id=file_unique_id  # ✅ NEW
            )
            await status_msg.edit_text(
                f"**✅ Upload Successful!** 🎉\n\n"
                f"📌 Title: `{title}`\n"
...
# ══════════════════════════════════════
# VIDEO HANDLER — ✅ DUPLICATE CHECK ADDED
# ══════════════════════════════════════
...
async def handle_video(client, message: Message):
    ...
    # ✅ NEW: MongoDB mein check karo kya pehle upload ho chuka hai
    if await db.is_already_uploaded(file_unique_id):
        # Purani video ka link dhundho
        existing = await client.db.col.find_one({"file_unique_id": file_unique_id})
        link_text = f"\n🔗 Link: {existing['yt_link']}" if existing and existing.get('yt_link') else "\n/links se dekho."
        await message.reply_text(
            f"⚠️ **Yeh video pehle se upload ho chuki hai!**\n\n"
            f"Dobara upload karne ki zaroorat nahi.{link_text}"
        )
        return

    # ✅ Check karo kya abhi queue mein hai (same session)
...
async def stop_cmd(client, message: Message):
    ...
    stop_flag = True
    await message.reply_text(
        "🚫 **Saare naye uploads band kar diye gaye!**\n\n"
        "Abhi jo chal rahi hain woh complete hongi.\n"
        "Dobara shuru karne ke liye `/resume` karo."
    )
...
async def resume_cmd(client, message: Message):
    global stop_flag
    stop_flag = False
    await message.reply_text("✅ **Uploads dobara shuru ho gaye!**\n\nAb users video bhej sakte hain.")
...
async def pending_cmd(client, message: Message):
    ...
    for i, p in enumerate(pending[:10], 1):
        text += f"{i}. `{p['title'][:40]}` — {p['size_mb']:.1f} MB\n"
    if count > 10:
        text += f"\n_...aur {count - 10} baki hain_"
    text += "\n\n`/retrypending` se dobara try karo."
    await message.reply_text(text)

@app.on_message(filters.command("retrypending") & filters.user(OWNER_ID))
async def retry_pending_cmd(client, message: Message):
    pending = await db.get_pending_uploads()
    if not pending:
        await message.reply_text("✅ Koi pending upload nahi hai!")
        return
    status_msg = await message.reply_text(
        f"🔄 **{len(pending)} pending uploads retry ho rahi hain...**"
    )
    success = 0
    failed = 0
    for p in pending:
        try:
            msg = await client.get_messages(p['chat_id'], p['message_id'])
            file = msg.video or msg.document
            if not file:
                failed += 1
                continue
            file_unique_id = getattr(file, 'file_unique_id', None) or str(p['message_id'])
            active_uploads[file_unique_id] = True
            await upload_queue.put((
                client, msg, status_msg, file_unique_id,
                p['title'], p['caption'], p['file_size'], p['size_mb']
            ))
            await db.delete_pending_upload(p['_id'])
            success += 1
        except Exception as e:
            logger.error(f"Retry error: {e}")
            await db.increment_pending_retry(p['_id'])
            failed += 1
    await status_msg.edit_text(
        f"**✅ Retry Complete!**\n\n"
        f"✅ Queue mein add: `{success}`\n"
        f"❌ Failed: `{failed}`"
    )
